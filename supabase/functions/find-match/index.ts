...
function checkCompatibility(userA: QueueEntry, userB: QueueEntry): boolean {
    ...
    if (!a_is_open && !b_is_open) {
        return a_interests.includes(userB.mbti_type) && b_interests.includes(userA.mbti_type);
    }
...
async function createMatch(supabaseAdmin: SupabaseClient, userA: QueueEntry, userB: QueueEntry) {
    ...
    if (deleteError) {
        ...
        console.error(`CRITICAL: Failed to remove users ${userA.user_id}, ${userB.user_id} from queue after creating chat ${chatId}. Error: ${deleteError.message}`);
    ...
    console.log(`Users ${userA.user_id} and ${userB.user_id} removed from queue.`);
...
serve(async (req) => {
  ...
  try {
    ...
    if (!userAData) {
        ...
        return new Response(JSON.stringify({ message: 'User not in queue or already matched.' }), {
    ...
    if (!potentialPartners || potentialPartners.length === 0) {
        ...
        return new Response(JSON.stringify({ message: 'No suitable match found yet.' }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200
        });
    }
    ...
    if (matchedPartner) {
        ...
        console.log(`Match successful for ${userA.user_id} and ${matchedPartner.user_id}. Chat ID: ${chatId}`);
        return new Response(JSON.stringify({ message: 'Match found!', chatId: chatId }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200
        });
    } else {
        console.log(`No compatible partners found for user ${userId} among waiting users.`);
        return new Response(JSON.stringify({ message: 'No compatible match found yet.' }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200
        });
    }
